...
@PerActivity
public class ShipmentLazerPresenter<V extends ShipmentLazerMvpView> extends BasePresenter<V> implements ShipmentLazerMvpPresenter<V> {

    private Sefer shipment;

    @Inject
    public ShipmentLazerPresenter(DataManager dataManager, CompositeDisposable compositeDisposable) {
...
//
//                        } else {
                            getMvpView().showShipmentLazer(response.getSefer());
//                        }
...
//                                    }
    ...
    @Override
    public void deleteBarcodes(String islemTipi) {
         int  tip =0 ;
        ...
        int finalTip = tip;
        getCompositeDisposable().add(getDataManager()
                .deleteBarcodesByType(tip)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<Boolean>() {
                    @Override
                    public void accept(Boolean aBoolean) throws Exception {
                        refreshList(finalTip);
                    }
                }, throwable -> {
